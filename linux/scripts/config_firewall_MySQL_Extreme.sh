#!/bin/bash
# Description:
# Setups the firewall rules for debian mysql
# Usage:
# ./<SCRIPT NAME>
# Debian MySQL iptables rules
# Supports ICMP, MySQL in
...
PAMgmt="172.20.242.150"
PAInternal="172.20.240.254"
...
iptables -A INPUT -p tcp -s "$ECom" --dport 3306 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
...
iptables -A OUTPUT -p tcp -d "$PAMgmt" --dport 443 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
