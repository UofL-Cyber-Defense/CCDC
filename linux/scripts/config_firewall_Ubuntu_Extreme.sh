#!/bin/bash
# Description:
# Setups the firewall rules for ubuntu
# Usage:
# ./<SCRIPT NAME>
# Ubuntu BIND iptables rules
# Supports DNS, ICMP in
...
iptables -A OUTPUT -p udp --sport 53 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -A OUTPUT -p tcp -d "$AD" --dport 53 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
