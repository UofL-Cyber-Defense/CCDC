#!/bin/bash
# Description:
# Setups the firewall rules for fedora web mail
# Usage:
# ./<SCRIPT NAME>
...
ip6tables -A OUTPUT -p udp -s "$AD" --sport 123 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
...
iptables -A INPUT -p tcp ! -s "$Inside" --dport 25 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -A INPUT -p tcp ! -s "$Inside" --dport 110 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -A INPUT -p tcp -s "$Inside" --dport 143 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
